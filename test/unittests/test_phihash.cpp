...
#include "helpers.hpp"
#include "phihash_test_vectors.hpp"

#include <gtest/gtest.h>
#include <array>

TEST(phihash, revision)
{
    static_assert(phihash::revision[0] == '0', "");
    static_assert(phihash::revision[1] == '.', "");
    static_assert(phihash::revision[2] == '9', "");
    static_assert(phihash::revision[3] == '.', "");
    static_assert(phihash::revision[4] == '4', "");
    EXPECT_EQ(phihash::revision, "0.9.4");
    EXPECT_EQ(phihash::revision, (std::string{"0.9.4"}));
}

TEST(phihash, l1_cache)
{
...
TEST(phihash, hash_empty)
{
    auto& context = get_ethash_epoch_context_0();

    const auto result = phihash::hash(context, 0, {}, 0);
...
TEST(phihash, generate_hash_test_cases)
{
    constexpr auto num_epochs = 2;

    using namespace phihash;
    hash256 h{};
